...
public class HavingTreeWalkerPostgres extends HavingTreeWalker{
	
	
	public HavingTreeWalkerPostgres(Stream strm, Unfolder unf, StarqlTransformator starqlTrans){
	...
	public String getHavingString(HvgTreeElem hElem, Boolean noConstraints){
		...
		case INDEXEDTRIPLE:
			...
			tmpSet.add( "SELECT * FROM(\r\n" + getUnf().unfoldDirect(((IndexedTriple)hElem), ((IndexedTriple)hElem).getTime(), true, getStrm().getFrom().getList()).replaceAll("(?m)^", "\t") + "   ) SUB_TRIPLE"+getnextOntopStrm()); 
