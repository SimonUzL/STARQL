...
public class HavingTreeWalkerLRB extends HavingTreeWalker{
	
	
	public HavingTreeWalkerLRB(Stream strm, Unfolder unf, StarqlTransformator starqlTrans){
	...
	public String getHavingString(HvgTreeElem hElem, Boolean noConstraints){
		...
		case INDEXEDTRIPLE:
			...
			for(Triple trple : ((IndexedTriple)hElem).getTriple()){
				tmpSet.add( "(\r\n" + getUnf().unfoldDirect(trple, ((IndexedTriple)hElem).getTime(), true, getStrm().getFrom().getList()).replaceAll("(?m)^", "\t") + "   ) SUB_TRIPLE"+getnextOntopStrm()); 
			}
