...
public class StarqlTransformatorSparkHistoric extends StarqlTransformator implements StarqlTransformatorInterface {
	...
	public StarqlTransformatorSparkHistoric(String starql, Unfolder unf) {
		setStarql(starql);
		setUnf(unf);
		try {
	...
	private String window_range(Stream strm, String viewString, String name, int range, int slide, Boolean fromStarql) {
		...
		if (getQinfo().getPulse() != null) {
			if (getQinfo().getPulse(strm.getPulse()).getStart() != null) {
				puls += "timestamp BETWEEN '"
						+ df.format(getQinfo().getPulse(strm.getPulse()).getStart().toDate()) + "'";
				if (getQinfo().getPulse(strm.getPulse()).getEnd() != null)
					puls += " AND ";
				puls += "'" + df.format(getQinfo().getPulse(strm.getPulse()).getEnd().toDate()) + "'";
			}
		}
		viewString += "CREATE time; \n"; 
		viewString += "SELECT DISTINCT timestamp FROM " + name + puls + " ORDER BY timestamp; \n";
		viewString += "WINDOW " + range + " " + slide + " " + name + "_wid; \n";
		return viewString;
	}
	...
	private String win(String name) {
		String str = "";
		str += genTableStr + name + "_win; \r\n"
				+ "SELECT wid, rank() OVER ( PARTITION BY wid ORDER BY timestamp ASC ) AS abox, timestamp AS t_wid FROM "
				+ name + "_wid;\r\n\r\n";
		return str;
	}
	...
	private String genStream(Stream strm, String name, Boolean fromStarql) {
		...
		if (fromStarql)
			from += "_starqlout";
		String str = "";
		str += "CREATE " + name + "_stream; \r\n" + "SELECT DISTINCT winseq.wid, winseq.abox, tble.*\r\n"
				+ "FROM " + name + "_win winseq, " + name + from + " tble\r\n"
				+ "WHERE winseq.t_wid = tble.timestamp \r\n";
		if (getQinfo().getPulse() != null) {
			if (getQinfo().getPulse(strm.getPulse()).getStart() != null) {
				str += "AND tble.timestamp BETWEEN '"
						+ df.format(getQinfo().getPulse(strm.getPulse()).getStart().toDate()) + "'";
				if (getQinfo().getPulse(strm.getPulse()).getEnd() != null)
					str += " AND ";
				str += "'" + df.format(getQinfo().getPulse(strm.getPulse()).getEnd().toDate()) + "'";
			}
		}
		str += "\r\nORDER BY wid, abox;\r\n";
		return str;
